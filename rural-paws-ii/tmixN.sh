#!/bin/bash
#script accepts a pcap file and returns the time needed for object to finish downloading ( from first styn till last ack)
...
do
        ...
        } | sort | awk -v st=$ST -v cmd_type=$1 -v TIME=$TIME '0.523154
        BEGIN {
                tot_ackd = tot_sent = 0
                rtt = 0
        }
